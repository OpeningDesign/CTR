\input{encab_A4}
\input{XC_page_layout_A4}
\usepackage{array, boldline, makecell, booktabs}
\newcommand\btrule[1]{\specialrule{#1}{0pt}{0pt}}
\usepackage{colortbl}
\usepackage{multicol,caption}               % three-column layout
%\usepackage[font={footnotesize}]{caption}
\usepackage{xcolor}
\newenvironment{Figure}
  {\par\medskip\noindent\minipage{\linewidth}}
  {\endminipage\par\medskip}
\usepackage{caption}
\usepackage[section]{placeins}
\usepackage{units}
%%\usepackage{draftwatermark}
%\maxdeadcycles=300

\title{Calculation Report: «1750 OX Residences - 1750 N Oxford Ave. - Eau Claire, WI» }
\author{XC structural engineering}
\date{\today}
\newcommand{\revision}{0.2}
\newcommand{\titdocum}{Calculation Report}
\begin{document}
\maketitle
\tableofcontents
\listoftables
\listoffigures
\cleardoublepage
\section{Introduction and scope}
\input{introduction.tex}
\section{Building codes}
\input{codes.tex}
\section{Loading criteria}
\input{loads.tex}
\section{Materials}
\input{materials.tex}
\section{Design and analysis software}
\input{software.tex}
\section{Load combinations}
\input{loadComb.tex}
\section{Wood framing}
\input{wood_framing.tex}
\section{Basement}
\input{basement.tex}
\section{Elevator shaft lintels}
\input{results_lintels/lintel_roof.tex}
\input{results_lintels/lintel_2floor.tex}
%\input{results_lintels/lintel_3floor.tex}
\input{results_lintels/lintel_1floor.tex}
\section{Balconies connection}
\input{balconies_connection.tex}

\newpage
\appendix
\section*{Appendices}
\addcontentsline{toc}{section}{Appendices}
\renewcommand{\thesubsection}{\Alph{subsection}}

\subsection{Loading criteria} \label{loadCrit}
\input{loads_annex/loads.tex}

\newpage
\subsection{Calculation results. Internal forces on columns}\label{sc_column_internal_forces}
\subsubsection{Ultimate limit states}
\input{annex_res_columns/ULS}
\clearpage
\newpage
\subsubsection{Serviceability limit states}
\input{annex_res_columns/SLS}
\end{document}
